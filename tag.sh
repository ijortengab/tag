    ...
    case $command in
        ...
        clear|c)
            if [ -f "$full_path" ];then
                cat "$full_path" >&2
                echo -ne "\e[93m"
                read -rsn1 -p 'File '"${basename}"' will be delete. Are you sure [y/n]: ' option
                echo -ne "\e[39m"
                if [[ $option == y ]];then
                    if [[ ! $dry_run == 1 ]];then
...
FindGenerator() {
    ...
    if [[ $process_file == 1 ]];then
        grep_command="grep${grep_arg_q}${grep_arg_E}${grep_arg_i}${grep_arg_w}${grep_arg}${grep_arg_file}"
    ...
    if [[ $process_dir == 1 ]];then
        ...
        grep_arg_file=' "$path"'
        grep_command="grep${grep_arg_q}${grep_arg_E}${grep_arg_i}${grep_arg_w}${grep_arg}${grep_arg_file}"
        filter_directory_command+='echo "$path" | sed -E "s|.*(\.[^.]+)$|\1|"'
        filter_directory_command='[[ $('"$filter_directory_command"') == ".tag" ]]'
